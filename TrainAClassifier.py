...
def train_a_model():
    for fold_num in range(pms.num_of_folds):
        ...
        val_dataset = My_Dataset(pms.train_path + f"/Val_f{fold_num + 1}.csv", transform=ToTensor())
        val_data_loader = torch.utils.data.DataLoader(val_dataset, batch_size=pms.batch_size, shuffle=True,
                                                      num_workers=10)
